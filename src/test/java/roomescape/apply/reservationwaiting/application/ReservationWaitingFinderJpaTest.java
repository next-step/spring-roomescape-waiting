...
class ReservationWaitingFinderJpaTest {
    ...
    @Test
    @DisplayName("사용자의 예약 대기들을 쿼리가 많이 나가며 가져올 수 있다.")
    void findReservationWaitingListByMemberId() {
        // given
    ...
    void findReservationWaitingListByMemberIdOneQuery() {
        ...
        // when
        var memberOneWaiting = reservationWaitingFinder.findReservationWaitingListByMemberIdOneQuery(member1.getId());
        var memberTwoWaiting = reservationWaitingFinder.findReservationWaitingListByMemberIdOneQuery(member2.getId());
    ...
    void findOldestReservationWaitingBy() {
        // given
        Theme theme = themeRepository.save(theme());
        ...
        // when
        var oldestReservationWaiting = reservationWaitingFinder.findOldestReservationWaitingBy(theme.getId(),
                "2099-12-02",
                reservationTime.getId());
        var notExistReservationWaiting = reservationWaitingFinder.findOldestReservationWaitingBy(theme.getId(),
                "2002-12-31",
                reservationTime.getId());
